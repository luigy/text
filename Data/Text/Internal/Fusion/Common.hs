...
-- cons !w (Stream next0 s0) = Stream next (C1 s0) -- (len+1)
--     where
...
-- snoc (Stream next0 xs0) w = Stream next (J xs0) --  (len+1)
--   where
...
-- tail (Stream next0 s0) = Stream next (C0 s0) -- (len-1)
--     where
--       next (C0 s) = case next0 s of
...
-- init (Stream next0 s0) = Stream next (Init0 s0) -- (len-1)
--     where
...
-- compareLengthI (Stream next s0) n =
...
-- map f (Stream next0 s0) = Stream next s0 -- len
--     where
...
-- caseConvert remap (Stream next0 s0) = Stream next (CC s0 '\0' '\0') -- len
--   where
--     next (CC s '\0' _) =
--         case next0 s of
--           Done       -> Done
...
-- toTitle (Stream next0 s0) = Stream next (CC (False :*: s0) '\0' '\0') -- len
--   where
...
-- scanl f z0 (Stream next0 s0) = Stream next (Scan1 z0 s0) -- (len+1) -- HINT maybe too low
--   where
--     {-# INLINE next #-}
...
-- takeWhile p (Stream next0 s0) = Stream next s0 -- len -- HINT maybe too high
--     where
--       {-# INLINE next #-}
--       next !s = case next0 s of
--                   Done    -> Done
...
-- dropWhile p (Stream next0 s0) = Stream next (L s0) -- len -- HINT maybe too high
--     where
--     {-# INLINE next #-}
...
-- filter p (Stream next0 s0) = Stream next s0 -- len -- HINT maybe too high
--   where
--     next !s = case next0 s of
--                 Done                   -> Done
--                 Skip    s'             -> Skip    s'
